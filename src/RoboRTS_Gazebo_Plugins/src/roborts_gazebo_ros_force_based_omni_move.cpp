#include "roborts_gazebo_ros_force_based_omni_move.h"
...
void GazeboRosForceBasedOmniMove::Load(physics::ModelPtr model, sdf::ElementPtr sdf) {
  ...
  if (!sdf->HasElement("publish_odom_tf")) {
    ...
    this->publish_odom_tf_ = sdf->GetElement("publish_odom_tf")->Get<bool>();
  }
  ...
  ros::SubscribeOptions sub_opt = ros::SubscribeOptions::create<geometry_msgs::Twist>(cmd_vel_topic_, 1,
                                                                                      boost::bind(&GazeboRosForceBasedOmniMove::CmdVelCallback, this, _1),
                                                                                      ros::VoidPtr(), &queue_);
  ...
  update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboRosForceBasedOmniMove::UpdateChild, this));
