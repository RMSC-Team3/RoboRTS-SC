...
class DecisionNode {
 public:
  DecisionNode() : global_planner_actionlib_client_("global_planner_node_action", true),
    ...
    ros::NodeHandle qyy;
    goal_sub_ = qyy.subscribe<geometry_msgs::PoseStamped>("center_pos", 1,
                                                          &DecisionNode::GoalCallback, this);
  ...
  void Execution(){
    //LOG_INFO<<"EXECUTION ENTERED";
    while(ros::ok()) {
      // LOG_INFO<<"WHILE LOOP ENTERED";
      if (rviz_goal_) {
        ROS_INFO("%d",rviz_goal_);
